    ...
    public record ResultDto(
        ...
        public static ResultDto from(ReconciliationResult r) {
            return new ResultDto(r.getSchemaName(), r.getTableName(), r.getSourceCount(),
                    r.getTargetCount(), r.getDifference(), r.getStatus(), r.getError());
        }
    }

    public record RunDto(
            UUID id, UUID projectId, String status, int totalTables, int mismatched,
        ...
        public static RunDto from(ReconciliationRun run, List<ReconciliationResult> results) {
            return new RunDto(run.getId(), run.getProjectId(), run.getStatus(), run.getTotalTables(),
                    run.getMismatched(), run.getStartedAt(), run.getFinishedAt(),
